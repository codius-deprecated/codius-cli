...
UploadCommand.prototype.run = function () {
  ...
  if (this.codius.config.config && this.codius.config.config.codiusHost) {
    hostUrl = this.codius.config.config.codiusHost;
  ...
  pack.pipe(gzip).pipe(request.post({
    ...
    request.post({
      ...
      winston.info('Contract ready at https://'+body.token+'.'+hostName+'/');
